...
resource "google_compute_router_interface" "if-tunnel0-to-vpc-demo" {
  ...
  vpn_tunnel = google_compute_vpn_tunnel.on-prem-tunnel0.id
  region = var.region1
}
...
resource "google_compute_router_interface" "if-tunnel1-to-vpc-demo" {
  ...
  vpn_tunnel = google_compute_vpn_tunnel.on-prem-tunnel1.id
  region = var.region1
}
...
resource "google_compute_router_peer" "bgp-vpc-demo-tunnel0" {
  ...
  router = google_compute_router.on-prem-router1.id
  interface = google_compute_router_interface.if-tunnel0-to-vpc-demo.id
...
resource "google_compute_router_peer" "bgp-vpc-demo-tunnel1" {
  ...
  router = google_compute_router.on-prem-router1.id
  interface = google_compute_router_interface.if-tunnel1-to-vpc-demo.id
